...
{
    public class UserWithEventsInvitationsAccessor : UserAccessorBase, IUserAccessor
    {
        public UserWithEventsInvitationsAccessor(MAppDbContext dbContext) : base(dbContext) { }
        public async override Task FindEntity(int entityID)
        {
            User = await dbContext.AllUsers.Include(x => x.OnlineEventsInvitations).Include(x => x.PhysicalEventsInvitations).FirstOrDefaultAsync(x => x.ID == entityID);
        }
